...
async def process_message(
    ...
    try:
        ...
        if not cluster_manager.list_clusters():
            cluster_manager.register_cluster("default", default_config)
        agent = cluster_manager.get_active_agent()
        # Ensure agent's client is initialized
        if agent and (not agent.client.v1 or not agent.client.apps_v1):
            agent.client.initialize()
        for action in actions:
            action_type = action.get("type")
            params = action.get("parameters", {})
            if action_type == "get_cluster_status" and agent and agent.client.v1:
                try:
                    nodes = agent.client.v1.list_node()
                    node_count = len(nodes.items)
                    pods = agent.client.v1.list_pod_for_all_namespaces()
            ...
            elif action_type == "list_pods" and agent and agent.client.v1:
                namespace = params.get("namespace", "default")
                try:
                    pods = agent.client.v1.list_namespaced_pod(namespace=namespace)
                    pod_names = [pod.metadata.name for pod in pods.items]
                    real_response = f"📦 Pods in {namespace}:\n" + "\n".join(pod_names)
                except Exception as e:
                    real_response = f"❌ Error getting pods: {str(e)}"
            elif action_type == "list_namespaces" and agent and agent.client.v1:
                try:
                    namespaces = agent.client.v1.list_namespace()
